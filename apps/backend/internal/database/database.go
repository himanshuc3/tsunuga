...
// Pooling to have certain open connections
// to avoid inflight TCP connections for each
// request
...
// Timeout for long requests in seconds
const DatabasePingTimeout = 10

// Context is basically prop drilling
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerConfig.LoggerService) (*Database, error) {
	...
	if cfg.Primary.Env == "local" {
		...
		// Chain tracers - New relic first, the local logging
